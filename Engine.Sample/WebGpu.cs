...
namespace Engine.Sample
{
	internal static class WebGpu
	{
		...
		public static int Run()
		{
			...
			GLFW.OnError += WebGpu.OnGlfwError;
			...
			{
				Width = WebGpu.windowWidth,
				Height = WebGpu.windowHeight,
			...
			var surface = WebGpu.CreateSurface(instance, window);
			...
			var device = adapter.RequestDevice(new DeviceDescriptor
			{
				...
				RequiredLimits = WebGpu.GetRequiredLimits(adapter),
			...
			var shader = WebGpuShaders.LoadFromFile(device, "shaders/shader.wgsl", ShaderType.WGSL, "Base Shader"u8);
		...
		[MustDisposeResource]
		private static Surface CreateSurface(Instance instance, Window window)
		{
		...
		private static Limits GetRequiredLimits(Adapter adapter)
		{
