...
Dictionary<char, Character> asciiChars = new();

const int asciiCharOffset = 32;
...
window.AddFramebufferSizeChanged(WindowFramebufferSizeCallback);
...
Buffer.BufferData(BufferTarget.Array, vertices, BufferUsage.StaticDraw);

// Write the index data to the GPU buffer.
indicesBuffer.Bind(BufferTarget.ElementArray);
Buffer.BufferData(BufferTarget.ElementArray, indices, BufferUsage.StaticDraw);
...
var texture = LoadTexture("./images/wall.jpg");
var texture2 = LoadTexture("./images/awesomeface.png");
...
var lastFrame = 0f;
var metrics = new PerformanceMetrics(stackalloc byte[32]);
...
while (!window.ShouldClose)
{
	...
	vertexArray.Bind();
	GL.DrawArrays(DrawMode.Triangles, 0, vertices.Length / 5);
...
void LoadAsciiCharacterMap()
{
	...
	for (var i = asciiCharOffset; i < 127; i++)
	{
		...
		asciiChars[(char)i] = new Character(
			texture,
...
void RenderText(Engine.OpenGL.Program shader,
				VertexArray vao,
				Buffer vbo,
				NativeString text,
				float x,
				...
				Vector3 color)
{
	...
	{
		if (!asciiChars.TryGetValue((char)@char, out var character))
		{
			// @todo Log error
			continue;
		}
...
static void WindowFramebufferSizeCallback(Window _, int width, int height) =>
